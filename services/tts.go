...
func GenerateSpeech(text string, outputPath string, voice string, rate string, pitch string) error {
	fmt.Printf("TTS: Starting generation - voice: %s, rate: %s, pitch: %s\n", voice, rate, pitch)
	...
	// Validate and format pitch parameter for edge-tts (expects format like "+0Hz", "-10Hz", "+25Hz")
	if pitch == "" {
		pitch = "+0Hz"
	}

	// Ensure pitch has the Hz suffix if not present
	if !strings.Contains(pitch, "Hz") {
		pitch = pitch + "Hz"
	}

	// First try edge-tts (high quality)
	fmt.Println("TTS: Attempting edge-tts...")
	err := tryEdgeTTS(text, outputPath, voice, rate, pitch)
	if err == nil {
		// edge-tts succeeded
	...
	return tryGTTTS(text, outputPath, voice, rate, pitch)
}

// tryEdgeTTS attempts to generate speech using edge-tts
func tryEdgeTTS(text string, outputPath string, voice string, rate string, pitch string) error {
	// Build edge-tts command following the official documentation format
	// Include voice, rate, and pitch parameters
	cmd := exec.Command("python", "-m", "edge_tts",
		"--voice", voice,
		fmt.Sprintf("--rate=%s", rate),
		fmt.Sprintf("--pitch=%s", pitch),
		"--text", text,
...
func tryGTTTS(text string, outputPath string, voice string, rate string, pitch string) error {
	...
	// gTTS doesn't support rate or pitch control, but we accept the parameters for future compatibility
	_ = rate  // Mark as used for future compatibility
	_ = pitch // Mark as used for future compatibility
...
`, text, lang, outputPath)
	...
	if info, err := os.Stat(outputPath); os.IsNotExist(err) {
		...
		return fmt.Errorf("gTTS created empty file: %s", outputPath)
	}
